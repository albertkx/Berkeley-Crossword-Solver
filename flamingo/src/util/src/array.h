/*
  $Id: array.h 5804 2010-10-23 21:22:15Z abehm $
...
public:
  ...
  Array() : capacity(10), elementNum(0), incrementalSize(10.f), reallocPolicy(REALLOC_ADD), elementSet(NULL) {
    init();
  };
  ...
  void erase(const unsigned position) {
    ...
    for (unsigned i = position + 1; i <= elementNum - 1; i ++) {
      data[i-1] = data[i];
    }
  ...
  void insert(const unsigned position, const T &element) {
    ...
    for (int i = elementNum - 1; i >= (int)position; i --) {
      data[i+1] = data[i];
    }
