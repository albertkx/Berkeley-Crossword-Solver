...
mergeWithoutDupls(vector<InvList*> &invLists,
  ...
  for(unsigned i = 0; i < numShortLists; i++) {
	...
	sc[id] = 1;
	candiCheck.push_back(id);
      }
    }
  }
  
  // guarantee short list candidates are sorted
  sort(candiCheck.begin(), candiCheck.end());
  ...
  for(unsigned i = 0; i < candiCheck.size(); i++) {
    unsigned id = candiCheck[i];
    ...
    unsigned targetCount = threshold - sc[id];
    unsigned count = 0;
    for(unsigned j = start; j < stop; j++) {	
      unsigned llindex = j - start;
      typename InvList::iterator start, end;
      expProbe(longListsPointers[llindex], invLists[j]->end(), start, end, id);
