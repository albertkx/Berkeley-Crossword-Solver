...
namespace Topk 
{
  namespace RoundRobin 
  {
    ...
    {
      float min;
      std::unordered_map<uint, std::list<_TopkEl>::iterator>::iterator ptr;
    };

    typedef std::list<_TopkEl> _TopkList;
    typedef std::unordered_map<uint, _TopkList::iterator> _TopkMap;
      ...
      while (!idxQue.lists.empty()) {
        ...
        for (std::list<std::list<range<uint*> >::iterator>::iterator 
          ...
          if (ls->_begin == ls->_end) { // skiped to the end
            ...
            std::unordered_map<uint*, uint>::iterator 
