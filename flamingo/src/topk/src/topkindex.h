...
namespace Topk
{
  ...
  */
  ...
  struct Index 
  {
    ...
    std::unordered_map<uint, range<uint*> > lists;
    ...
    const GramGen &gramGen)
  {
    ...
    std::unordered_map<uint, std::vector<uint>* > idx;
    ...
    for (uint id = 0; data != data_end; ++data, id++) {
      ...
      for (std::set<uint>::const_iterator gram = grams.begin(); 
        ...
        std::unordered_map<uint, std::vector<uint>*>::const_iterator pos = 
          idx.find(*gram);
        if (pos == idx.end()) {
    ...
    for(std::unordered_map<uint, std::vector<uint>*>::const_iterator pos = 
